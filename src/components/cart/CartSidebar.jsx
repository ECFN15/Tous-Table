...
const CartSidebar = ({ isOpen, onClose, cartItems, onRemoveItem, totalPrice, onCheckout, interacted, darkMode, activeDesignId }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[120] ${isOpen ? 'visible' : 'invisible delay-700'}`}>
            ...
            {/* Sidebar Panel */}
            <div
                className={`absolute right-0 top-0 bottom-0 w-full md:w-[500px] shadow-2xl transition-all ${baseTransition} transform-gpu p-8 flex flex-col ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'} ${bgClass}`}
            >
