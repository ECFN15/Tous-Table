...
const ShopSidebar = ({
    ...
    return (
        <>
            ...
            `}>
                <div className="space-y-4 p-4.5 pb-5 [@media(max-height:720px)]:space-y-3 [@media(max-height:720px)]:p-3.5 [@media(max-height:720px)]:pb-4 xl:p-4.5 xl:pb-5">
                    <div className={`rounded-[20px] border p-3.5 [@media(max-height:720px)]:p-3 ${darkMode ? 'border-white/10 bg-white/[0.035]' : 'border-[#c79b5d]/24 bg-white/55'}`}>
                        <div className="flex items-center justify-between gap-4">
                            <div>
                                <span className={`text-[8.5px] font-black uppercase tracking-[0.28em] ${darkMode ? 'text-stone-500' : 'text-stone-500'}`}>
                                    Comptoir
                                </span>
                                <p className={`mt-1.5 font-serif text-[1.72rem] leading-none [@media(max-height:720px)]:mt-1 [@media(max-height:720px)]:text-[1.55rem] ${darkMode ? 'text-white' : 'text-stone-900'}`}>
                                    {filteredProductCount}
                                </p>
                            </div>
                            <div className={`flex h-9 w-9 items-center justify-center rounded-full [@media(max-height:720px)]:h-8 [@media(max-height:720px)]:w-8 ${darkMode ? 'bg-amber-500/15 text-amber-300' : 'bg-amber-700/10 text-amber-800'}`}>
                                <SlidersHorizontal size={17} strokeWidth={1.8} />
                            </div>
                        </div>
                        <p className={`mt-2 text-[10px] leading-relaxed [@media(max-height:720px)]:hidden ${darkMode ? 'text-stone-500' : 'text-stone-500'}`}>
                    ...
                    >
                        <div className={`
                            flex items-center justify-between rounded-[15px] px-3.5 py-2.5 [@media(max-height:720px)]:px-3 [@media(max-height:720px)]:py-2
                    ...
                    <nav className="space-y-1.5 [@media(max-height:720px)]:space-y-1">
                        <p className={`px-3.5 text-[8.5px] font-black uppercase tracking-[0.26em] [@media(max-height:720px)]:px-3 ${darkMode ? 'text-stone-600' : 'text-stone-500'}`}>
                            Familles
                        </p>
                            ...
                            >
                                ...
                                `}>
                                    <div className="min-w-0 flex-1 space-y-0.5">
                                        <h3 className={`
                                            truncate text-[13.5px] font-bold tracking-tight transition-colors duration-500 [@media(max-height:720px)]:text-[13px]
                                            ${activeCategory === category.id
                                                ? (darkMode ? 'text-amber-400' : 'text-amber-800')
                                                : (darkMode ? 'text-white group-hover:text-amber-500' : 'text-stone-900 group-hover:text-amber-700')
                                            }
                                        ...
                                        <p className={`
                                            ...
                                            truncate text-[9px] font-medium uppercase tracking-wider
                                            ${activeCategory === category.id
                                    ...
                                    <span className={`ml-3 flex h-6 min-w-6 items-center justify-center rounded-full px-1.5 text-[9px] font-black [@media(max-height:720px)]:ml-2.5 [@media(max-height:720px)]:h-5 [@media(max-height:720px)]:min-w-5 [@media(max-height:720px)]:text-[8.5px] ${activeCategory === category.id
                    ...
                    <div className="space-y-2 [@media(max-height:720px)]:space-y-2">
                        <p className={`px-3.5 text-[8.5px] font-black uppercase tracking-[0.26em] [@media(max-height:720px)]:px-3 ${darkMode ? 'text-stone-600' : 'text-stone-500'}`}>
                            Prix indicatif
                        </p>
                        <div className="grid grid-cols-2 gap-1.5 [@media(max-height:720px)]:gap-1.5">
                            {priceFilters.map((filter) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`rounded-full border px-3 py-2 text-[9px] font-black uppercase tracking-[0.1em] transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] [@media(max-height:720px)]:px-2.5 [@media(max-height:720px)]:py-1.5 [@media(max-height:720px)]:text-[8.5px] ${isActive
