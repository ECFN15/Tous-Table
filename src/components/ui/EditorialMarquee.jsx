...
const MarqueeRow = ({
    ...
    useEffect(() => {
        return scrollVelocity.onChange((v) => {
            if (!activeRef.current || Math.abs(v) <= 1) return;
    ...
    useEffect(() => {
        ...
        if (!row || typeof IntersectionObserver === 'undefined') return undefined;
    ...
    }, [baseSpeed, direction, scrollVelocity]);
                            ...
                            <React.Fragment key={idx}>
                                {item.type === 'text' ? (
                                    ...
                                    <div className="h-[95%] sm:h-[105%] aspect-square rounded-full border-[4px] sm:border-[6px] md:border-[8px] lg:border-[12px] border-[#FDFDFD] overflow-hidden shadow-[0_10px_40px_-10px_rgba(0,0,0,0.3)] mx-3 sm:mx-5 lg:mx-8 flex-shrink-0 relative transform hover:scale-105 transition-transform duration-500 ring-1 ring-black/5">
                                        <img src={item.content} alt="decor" loading="lazy" decoding="async" className="w-full h-full object-cover" />
            ...
            try {
                ...
                const buildRow = (rowNum) => {
                    ...
                    for (let i = 1; i <= 3; i += 1) {
                        ...
                        const fallback = getDefault(rowNum, i);
                        rowItems.push({ type: 'text', content: data[`${key}_text`]?.word || fallback.word });
                        rowItems.push({ type: 'image', content: data[key] || fallback.img });
                    }
                    return rowItems;
                };
