...
const CartSidebar = ({ isOpen, onClose, cartItems, onRemoveItem, totalPrice, onCheckout, interacted, darkMode }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[120] ${isOpen ? 'visible' : 'invisible delay-700'}`}>
            ...
            {/* Sidebar Panel */}
            <div
                className={`absolute right-0 top-0 bottom-0 w-full md:w-[500px] shadow-2xl transition-all ${baseTransition} transform-gpu p-8 flex flex-col ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'} ${darkMode ? 'bg-stone-900 border-l border-stone-800 text-white' : 'bg-[#FAF9F6] text-stone-900'}`}
            >

                {/* Header */}
                <div className={`flex justify-between items-center mb-10 border-b pb-6 ${darkMode ? 'border-stone-800' : 'border-stone-200'}`}>
                    <div className="flex items-center gap-3">
                        <ShoppingBag size={24} className={darkMode ? 'text-white' : 'text-stone-900'} />
                ...
                <div className="flex-1 overflow-y-auto space-y-6 pr-2 scrollbar-thin scrollbar-thumb-stone-200">
                    {cartItems.length === 0 ? (
                        ...
                        cartItems.map((item) => (
                            <div key={item.id} className={`flex gap-4 p-4 rounded-2xl border shadow-sm relative group animate-in slide-in-from-right-8 duration-500 ${darkMode ? 'bg-stone-800/50 border-stone-700' : 'bg-white border-stone-100'}`}>
                                <img
                                    src={item.image}
                                    alt={item.name}
                                ...
                                <div className="flex-1 flex flex-col justify-center">
                                    <h3 className={`font-bold leading-tight ${darkMode ? 'text-white' : 'text-stone-900'}`}>{item.name}</h3>
                ...
                {cartItems.length > 0 && (
                    <div className={`pt-8 mt-4 border-t space-y-6 ${darkMode ? 'border-stone-800' : 'border-stone-200'}`}>
                        <div className="flex justify-between items-end">
                            ...
                            <span className={`text-4xl font-black tracking-tighter ${darkMode ? 'text-white' : 'text-stone-900'}`}>{totalPrice} €</span>
                        </div>
                        <button
                            onClick={onCheckout}
                            className={`w-full py-5 rounded-xl font-black uppercase text-xs tracking-[0.2em] transition-all flex items-center justify-center gap-3 shadow-xl group ${darkMode ? 'bg-white text-stone-900 hover:bg-amber-500 hover:text-white' : 'bg-stone-900 text-white hover:bg-amber-600'}`}
                        >
